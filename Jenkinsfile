pipeline {
    ...
    stages {
        ...
        stage('key'){
            steps {
                sh label: '', script: ''' sed -i \'s/active/$access_key/g\' /var/lib/jenkins/workspace/terragods/variables.tf
sed -i \'s/security/$secret_key/g\' /var/lib/jenkins/workspace/terragods/variables.tf'''
                  }
            }
